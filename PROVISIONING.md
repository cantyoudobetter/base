# How to provision a new environment for dpw
...
* Enter domains to secure (for example): `dpw.com`
* Enter webroot path: `/home/deployer/apps/dpw/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/dpw.com/fullchain.pem /etc/ssl/dpw.crt
ln -s /etc/letsencrypt/live/dpw.com/privkey.pem /etc/ssl/dpw.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/dpw/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
52 1,13 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/dpw/shared/public/
```
